...
func (this *SignerSuite) new(urlSets []util.URLSet) *Signer {
	handler, err := New(pkgt.Certs[0], pkgt.Key, urlSets, &rtv.RTVCache{}, func() bool { return this.shouldPackage }, nil, true, pkgt.ForwardedRequestHeaders)
...
func (this *SignerSuite) getFRH(t *testing.T, handler pkgt.AlmostHandler, target string, host string) *http.Response {
	return pkgt.GetBHHP(t, handler, target, host, http.Header{
		"AMP-Cache-Transform": {"google"}, "Accept": {"application/signed-exchange;v=" + accept.AcceptedSxgVersion}, "X-Foo": {"foo"}, "X-Bar": {"bar"}},  httprouter.Params{})
}
...
func (this *SignerSuite) TestFetchSignWithForwardedRequestHeaders() {
	...
	this.fakeHandler = func(resp http.ResponseWriter, req *http.Request) {
		// Host and X-Foo headers are forwarded with pkgt.ForwardedRequestHeaders
	...
	resp := this.getFRH(this.T(), this.new(urlSets),
		"/priv/doc?fetch="+url.QueryEscape(this.httpURL()+fakePath)+
			"&sign="+url.QueryEscape(this.httpSignURL_CertSubjectCN()+fakePath), "www.example.com")
	this.Assert().Equal(fakePath, this.lastRequest.URL.String())
	this.Assert().Equal(userAgent, this.lastRequest.Header.Get("User-Agent"))
	this.Assert().Equal("1.1 amppkg", this.lastRequest.Header.Get("Via"))
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
