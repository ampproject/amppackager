...
func (this *SignerSuite) new(urlSets []util.URLSet) *signer.Signer {
	forwardedRequestHeaders := []string{"Host", "X-Foo"}
	handler, err := signer.New(pkgt.Certs[0], pkgt.Key, urlSets, &rtv.RTVCache{}, func() bool { return this.shouldPackage }, nil, true, forwardedRequestHeaders)
...
func (this *SignerSuite) get(t *testing.T, handler http.Handler, target string) *http.Response {
	return this.getP(t, handler, target, map[string]string{})
}

func (this *SignerSuite) getP(t *testing.T, handler http.Handler, target string, params map[string]string) *http.Response {
...
func (this *SignerSuite) getFRH(t *testing.T, handler http.Handler, target string, host string, header http.Header) *http.Response {
	return pkgt.GetBHHP(t, handler, target, host, header, map[string]string{})
}
...
func (this *SignerSuite) SetupSuite() {
	...
	this.httpsClient.CheckRedirect = signer.NoRedirects
}
...
func (this *SignerSuite) SetupTest() {
	...
	signer.GetTransformerRequest = func(r *rtv.RTVCache, s, u string) *rpb.Request {
...
func (this *SignerSuite) TestSimple() {
	...
	this.Assert().Equal(signer.UserAgent, this.lastRequest.Header.Get("User-Agent"))
	...
	binary.Write(&payloadPrefix, binary.BigEndian, uint64(signer.MIRecordSize))
...
func (this *SignerSuite) TestSignAsPathParam() {
	...
	resp := this.getP(this.T(), this.new(urlSets), `/priv/doc/`, map[string]string{"signURL": this.httpsURL() + fakePath})
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
...
func (this *SignerSuite) TestSignAsPathParamWithUnusualPctEncoding() {
	...
	mux := mux.New(nil, this.new(urlSets), nil)
	rec := httptest.NewRecorder()
	req := httptest.NewRequest("", `/priv/doc/` + this.httpsURL() + fakePath + `%2A`, nil)
	println("requri =", req.RequestURI)
	req.Header.Add("AMP-Cache-Transform", "google")
	req.Header.Add("Accept", "application/signed-exchange;v=" + accept.AcceptedSxgVersion)
	println("hi there")
