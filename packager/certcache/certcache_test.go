...
func (this *CertCacheSuite) TestOCSP1() {
	// Verify it gets included in the cert-chain+cbor payload.
	resp := pkgt.Get(this.T(), this.mux(), "/amppkg/cert/"+pkgt.CertName)
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
	// 302400 is 3.5 days. max-age is slightly less because of the time between fake OCSP generation and cert-chain response.
...
func (this *CertCacheSuite) TestOCSP2() {
	// Verify it gets included in the cert-chain+cbor payload.
	resp := pkgt.Get(this.T(), this.mux(), "/amppkg/cert/"+pkgt.CertName)
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
	// 302400 is 3.5 days. max-age is slightly less because of the time between fake OCSP generation and cert-chain response.
...
func (this *CertCacheSuite) TestOCSP3() {
	// Verify it gets included in the cert-chain+cbor payload.
	resp := pkgt.Get(this.T(), this.mux(), "/amppkg/cert/"+pkgt.CertName)
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
	// 302400 is 3.5 days. max-age is slightly less because of the time between fake OCSP generation and cert-chain response.
...
func (this *CertCacheSuite) TestOCSP4() {
	// Verify it gets included in the cert-chain+cbor payload.
	resp := pkgt.Get(this.T(), this.mux(), "/amppkg/cert/"+pkgt.CertName)
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
	// 302400 is 3.5 days. max-age is slightly less because of the time between fake OCSP generation and cert-chain response.
...
func (this *CertCacheSuite) TestOCSP5() {
	// Verify it gets included in the cert-chain+cbor payload.
	resp := pkgt.Get(this.T(), this.mux(), "/amppkg/cert/"+pkgt.CertName)
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
	// 302400 is 3.5 days. max-age is slightly less because of the time between fake OCSP generation and cert-chain response.
...
func (this *CertCacheSuite) TestOCSP6() {
	// Verify it gets included in the cert-chain+cbor payload.
	resp := pkgt.Get(this.T(), this.mux(), "/amppkg/cert/"+pkgt.CertName)
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
	// 302400 is 3.5 days. max-age is slightly less because of the time between fake OCSP generation and cert-chain response.
...
func (this *CertCacheSuite) TestOCSP7() {
	// Verify it gets included in the cert-chain+cbor payload.
	resp := pkgt.Get(this.T(), this.mux(), "/amppkg/cert/"+pkgt.CertName)
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
	// 302400 is 3.5 days. max-age is slightly less because of the time between fake OCSP generation and cert-chain response.
...
func (this *CertCacheSuite) TestOCSP8() {
	// Verify it gets included in the cert-chain+cbor payload.
	resp := pkgt.Get(this.T(), this.mux(), "/amppkg/cert/"+pkgt.CertName)
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
	// 302400 is 3.5 days. max-age is slightly less because of the time between fake OCSP generation and cert-chain response.
...
func (this *CertCacheSuite) TestOCSP9() {
	// Verify it gets included in the cert-chain+cbor payload.
	resp := pkgt.Get(this.T(), this.mux(), "/amppkg/cert/"+pkgt.CertName)
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
	// 302400 is 3.5 days. max-age is slightly less because of the time between fake OCSP generation and cert-chain response.
...
func (this *CertCacheSuite) TestOCSPA() {
	// Verify it gets included in the cert-chain+cbor payload.
	resp := pkgt.Get(this.T(), this.mux(), "/amppkg/cert/"+pkgt.CertName)
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
	// 302400 is 3.5 days. max-age is slightly less because of the time between fake OCSP generation and cert-chain response.
...
func (this *CertCacheSuite) TestOCSPB() {
	// Verify it gets included in the cert-chain+cbor payload.
	resp := pkgt.Get(this.T(), this.mux(), "/amppkg/cert/"+pkgt.CertName)
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
	// 302400 is 3.5 days. max-age is slightly less because of the time between fake OCSP generation and cert-chain response.
...
func (this *CertCacheSuite) TestOCSPC() {
	// Verify it gets included in the cert-chain+cbor payload.
	resp := pkgt.Get(this.T(), this.mux(), "/amppkg/cert/"+pkgt.CertName)
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
	// 302400 is 3.5 days. max-age is slightly less because of the time between fake OCSP generation and cert-chain response.
...
func (this *CertCacheSuite) TestOCSPD() {
	// Verify it gets included in the cert-chain+cbor payload.
	resp := pkgt.Get(this.T(), this.mux(), "/amppkg/cert/"+pkgt.CertName)
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
	// 302400 is 3.5 days. max-age is slightly less because of the time between fake OCSP generation and cert-chain response.
...
func (this *CertCacheSuite) TestOCSPE() {
	// Verify it gets included in the cert-chain+cbor payload.
	resp := pkgt.Get(this.T(), this.mux(), "/amppkg/cert/"+pkgt.CertName)
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
	// 302400 is 3.5 days. max-age is slightly less because of the time between fake OCSP generation and cert-chain response.
...
func (this *CertCacheSuite) TestOCSPF() {
	// Verify it gets included in the cert-chain+cbor payload.
	resp := pkgt.Get(this.T(), this.mux(), "/amppkg/cert/"+pkgt.CertName)
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
	// 302400 is 3.5 days. max-age is slightly less because of the time between fake OCSP generation and cert-chain response.
...
func (this *CertCacheSuite) TestOCSPG() {
	// Verify it gets included in the cert-chain+cbor payload.
	resp := pkgt.Get(this.T(), this.mux(), "/amppkg/cert/"+pkgt.CertName)
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
	// 302400 is 3.5 days. max-age is slightly less because of the time between fake OCSP generation and cert-chain response.
...
func (this *CertCacheSuite) TestOCSPH() {
	// Verify it gets included in the cert-chain+cbor payload.
	resp := pkgt.Get(this.T(), this.mux(), "/amppkg/cert/"+pkgt.CertName)
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
	// 302400 is 3.5 days. max-age is slightly less because of the time between fake OCSP generation and cert-chain response.
...
func (this *CertCacheSuite) TestOCSPI() {
	// Verify it gets included in the cert-chain+cbor payload.
	resp := pkgt.Get(this.T(), this.mux(), "/amppkg/cert/"+pkgt.CertName)
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
	// 302400 is 3.5 days. max-age is slightly less because of the time between fake OCSP generation and cert-chain response.
...
func (this *CertCacheSuite) TestOCSPJ() {
	// Verify it gets included in the cert-chain+cbor payload.
	resp := pkgt.Get(this.T(), this.mux(), "/amppkg/cert/"+pkgt.CertName)
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
	// 302400 is 3.5 days. max-age is slightly less because of the time between fake OCSP generation and cert-chain response.
...
func (this *CertCacheSuite) TestOCSPK() {
	// Verify it gets included in the cert-chain+cbor payload.
	resp := pkgt.Get(this.T(), this.mux(), "/amppkg/cert/"+pkgt.CertName)
	this.Assert().Equal(http.StatusOK, resp.StatusCode, "incorrect status: %#v", resp)
	// 302400 is 3.5 days. max-age is slightly less because of the time between fake OCSP generation and cert-chain response.
